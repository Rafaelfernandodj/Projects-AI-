      ...
      <div className="bg-brand-dark-light border-t sm:border border-brand-dark-border w-full max-w-2xl rounded-t-2xl sm:rounded-2xl flex flex-col max-h-[85vh] sm:max-h-[90vh] overflow-hidden shadow-2xl animate-in fade-in slide-in-from-bottom-8 sm:slide-in-from-bottom-0 sm:zoom-in-95 duration-250">
        ...
        <div className="p-4 sm:p-6 border-b border-brand-dark-border flex items-center justify-between">
          <div className="flex items-center space-x-3">
            <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-xl bg-brand-green/10 flex items-center justify-center shrink-0">
              <BookOpen className="text-brand-green-glow animate-pulse" size={18} />
            </div>
            <div>
              <h2 className="text-base sm:text-xl font-bold text-brand-text">Planos de Estudo Personalizados</h2>
              <p className="text-[10px] sm:text-xs text-gray-400">Pratique com cenários focados e preferências de suporte</p>
            </div>
          </div>
          <button
            onClick={onClose}
            className="p-1.5 sm:p-2 text-gray-400 hover:text-white hover:bg-white/5 rounded-xl transition-colors"
          >
            <X size={18} />
          </button>
        </div>
        ...
        <div className="p-3 sm:p-4 mx-4 sm:mx-6 mt-3 sm:mt-4 bg-brand-dark rounded-xl border border-brand-green/20">
          <div className="flex items-center justify-between flex-wrap gap-2">
            <div className="flex items-center space-x-3">
              <span className="flex h-2 w-2 relative">
                ...
                <span className={`relative inline-flex rounded-full h-2 w-2 ${activePlan ? "bg-brand-green" : "bg-gray-500"}`}></span>
              </span>
              <div>
                <p className="text-[10px] text-gray-400 uppercase font-bold tracking-wider">Plano Ativo no Live Mode</p>
            ...
            {activePlan && (
              <button
                ...
                className="px-2 py-1 bg-red-500/10 border border-red-500/20 text-red-400 text-[10px] sm:text-xs font-semibold rounded-lg hover:bg-red-500/20 transition-colors"
              >
                Limpar Ativo
              </button>
            )}
          ...
          {activePlan && (
            <div className="mt-3 border-t border-brand-dark-border pt-2 flex flex-col md:flex-row md:items-center justify-between gap-3">
              <div className="space-y-1">
                <span className="text-[9px] py-0.5 px-1.5 bg-brand-green/10 text-brand-green-glow rounded font-bold border border-brand-green/20">
                  Idioma: {activePlan.languagePreference === "portuguese" ? "Explicações em Português" : activePlan.languagePreference === "mixed" ? "Misturado (PT/EN)" : "100% Inglês"}
                </span>
                <p className="text-[11px] text-gray-400 mt-1 max-w-md italic line-clamp-2 md:line-clamp-none">{activePlan.scenario}</p>
              </div>

              {/* Language Preferences */}
              <div className="flex items-center space-x-0.5 bg-brand-dark-light border border-brand-dark-border rounded-lg p-0.5 self-start shrink-0">
                <button
                  onClick={() => handleChangeLanguagePreference("portuguese")}
                  className={`px-1.5 py-0.5 text-[10px] rounded-md font-medium transition-colors ${activePlan.languagePreference === "portuguese" ? "bg-brand-green text-brand-dark" : "text-gray-400 hover:text-white"}`}
                >
                  PT
                </button>
                <button
                  onClick={() => handleChangeLanguagePreference("mixed")}
                  className={`px-1.5 py-0.5 text-[10px] rounded-md font-medium transition-colors ${activePlan.languagePreference === "mixed" ? "bg-brand-green text-brand-dark" : "text-gray-400 hover:text-white"}`}
                >
                  Mix
                </button>
                <button
                  onClick={() => handleChangeLanguagePreference("english")}
                  className={`px-1.5 py-0.5 text-[10px] rounded-md font-medium transition-colors ${activePlan.languagePreference === "english" ? "bg-brand-green text-brand-dark" : "text-gray-400 hover:text-white"}`}
                >
                  EN
                </button>
              </div>
        ...
        <div className="px-4 sm:px-6 mt-3 sm:mt-4 flex border-b border-brand-dark-border gap-4">
          <button
            ...
            className={`pb-2 text-xs sm:text-sm font-semibold transition-colors relative ${activeTab === "templates" ? "text-brand-green-glow" : "text-gray-400 hover:text-white"}`}
          >
            Sugeridos (Templates)
            {activeTab === "templates" && (
          ...
          <button
            ...
            className={`pb-2 text-xs sm:text-sm font-semibold transition-colors relative flex items-center gap-1.5 ${activeTab === "custom" ? "text-brand-green-glow" : "text-gray-400 hover:text-white"}`}
          >
            Gerados Pelo Chat
            {dbPlans.length > 0 && (
              <span className="h-1 w-1 rounded-full bg-brand-green-glow animate-pulse"></span>
            )}
            {activeTab === "custom" && (
              <span className="absolute bottom-0 left-0 right-0 h-0.5 bg-brand-green-glow"></span>
            )}
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-2 sm:space-y-3 scrollbar-thin scrollbar-thumb-brand-green/20 scrollbar-track-transparent">
          {loading ? (
            <div className="flex flex-col items-center justify-center py-10 space-y-2">
              <div className="w-6 h-6 rounded-full border-2 border-brand-green border-t-transparent animate-spin"></div>
              <p className="text-[10px] text-gray-400">Buscando seus planos...</p>
            </div>
          ) : currentPlans.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-12 text-center">
              <Layers className="text-gray-600 mb-2" size={30} />
              <p className="text-xs text-brand-text font-bold">Nenhum plano gerado ainda</p>
                  ...
                  <div className="flex items-start justify-between gap-3">
                    <div className="space-y-1 min-w-0 flex-1">
                      <div className="flex items-center space-x-2">
                        <h4 className="font-bold text-brand-text text-xs sm:text-sm group-hover:text-brand-green-glow transition-colors truncate">
                          {plan.title}
                        </h4>
                        {plan.id.startsWith("tpl_") && (
                          <span className="text-[8px] font-bold px-1 py-0.2 bg-white/5 text-gray-400 border border-brand-dark-border rounded">
                            PRESET
                          </span>
                        )}
                      </div>
                    ...
                    <div className="flex items-center space-x-2 shrink-0">
                      {isSelected ? (
                        <div className="w-5 h-5 rounded-full bg-brand-green flex items-center justify-center text-brand-dark shrink-0">
                          <Check size={12} strokeWidth={3} />
                        </div>
                      ) : (
                        <div className="w-5 h-5 rounded-full border border-gray-600 group-hover:border-brand-green flex items-center justify-center text-transparent group-hover:text-brand-green-glow text-[10px] transition-all shrink-0">
                          ✓
                        </div>
                      )}
                      ...
                      {!plan.id.startsWith("tpl_") && (
                        <button
                          ...
                          className="p-1 text-gray-500 hover:text-red-400 rounded hover:bg-white/5 opacity-0 group-hover:opacity-100 transition-opacity shrink-0"
                          title="Remover plano"
                        >
                          <Trash2 size={12} />
                        </button>
        ...
        <div className="p-3 sm:p-4 border-t border-brand-dark-border bg-brand-dark/50 flex justify-between items-center px-4 sm:px-6">
          <div className="flex items-center space-x-1.5 text-[10px] sm:text-xs text-brand-green-glow font-medium min-w-0">
            <Sparkles size={12} className="shrink-0" />
            <span className="truncate">Peça planos novos no Chat de Texto!</span>
          </div>
          <button
            onClick={onClose}
            className="px-3 py-1.5 sm:px-4 sm:py-2 bg-brand-green text-brand-dark rounded-xl text-[11px] sm:text-xs font-bold hover:brightness-110 active:scale-95 transition-all shrink-0"
          >
            Confirmar
          </button>
        </div>
